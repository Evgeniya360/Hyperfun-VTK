...
vtkCxxRevisionMacro(SampFunc, "$Revision: 1.78 $");
vtkStandardNewMacro(SampFunc);
vtkCxxSetObjectMacro(SampFunc,ImplicitFunction,vtkImplicitFunction);
...
void SampFunc::SetSampleDimensions(int i, int j, int k)
{
  int dim[3];
...
// Specify the dimensions of the data on which to sample.
void SampFunc::SetSampleDimensions(int dim[3])
{
...
int SampFunc::RequestInformation (
...
void SampFunc::ExecuteData(vtkDataObject *outp)
{
  vtkIdType idx, i, j, k;
  ...
  for ( idx=0, k=extent[4]; k <= extent[5]; k++ )
    ...
    for ( j=extent[2]; j <= extent[3]; j++ )
      ...
      for ( i=extent[0]; i <= extent[1]; i++ )
		...
		//atr=((ImpFunc*)ImplicitFunction)->itsSAttributesM[FunAttNum-1];
		//l=(char)s;
		if(FunAttNum==0)
		...
		else
		{
			if(FunAttNum==-1)
			{
				...
				color[0]=((ImpFunc*)ImplicitFunction)->itsSAttributesM[0];
				color[1]=((ImpFunc*)ImplicitFunction)->itsSAttributesM[1];
				color[2]=((ImpFunc*)ImplicitFunction)->itsSAttributesM[2];

				scalval=0.0;
				if (color[0]>0.0)
				{
					ex=0;
					if (color[1]>0.0) {scalval=1.0; ex=1;}
					if (color[2]>0.0) {scalval=5.0; ex=1;}
					if (ex==0) scalval=0.0;
//printf("color= %lf , %lf, %lf \n",color[0],color[1],color[2]);
				...
				newScalars->SetTuple1(idx++,scalval);
...
unsigned long SampFunc::GetMTime()
{
...
void SampFunc::Cap(vtkDataArray *s)
{
  int i,j,k,extent[6];
  vtkIdType idx;
...
void SampFunc::SetScalars(vtkDataArray *da)
{
  if (da)
    {
...
void SampFunc::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void SampFunc::ReportReferences(vtkGarbageCollector* collector)
{
